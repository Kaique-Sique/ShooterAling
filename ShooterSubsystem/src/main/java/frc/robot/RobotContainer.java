...
import frc.robot.commands.shooterCmds.ShooterOutCmd;
...
public class RobotContainer {
  ...
  private void configureBindings() 
  {
    ...
    .whileTrue(
        new RunCommand(
                ()-> baseShooter.setPIDPosition(0), baseShooter)
        .alongWith(
          new RunCommand(()-> capoShooter.setTargetPosePID(0), capoShooter)))
    .onFalse(
        new InstantCommand(
                ()-> baseShooter.stopMotor(), baseShooter)
        .andThen(
    ...
    m_driverController.povDown()
      ...
      .onFalse(new InstantCommand(()-> shooterSubsys.stopMotors()));
    
    // trigger digital switch
    Trigger sensorShoot = new Trigger(()->digitalSwitch.get());
    sensorShoot.onTrue(new ShootCmd());
  }
